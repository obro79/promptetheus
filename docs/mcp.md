...
evidence for the configured Supabase project on demand.
...
- the Supabase access scope for the server
...
The hosted MCP server should use read-only, project-scoped Supabase access by
default. A generated URL includes the Promptetheus workspace and Supabase
`project_ref`, so evidence reads are scoped to that project. The SDK and MCP
client config must not receive Supabase service-role keys.
