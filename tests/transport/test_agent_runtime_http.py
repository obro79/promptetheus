...
from promptetheus.config import Config, override_config, reset_config
...
def test_endpoint_with_api_suffix_does_not_duplicate_api(monkeypatch):
    ...
    runtime = AgentRuntime("sess_1", endpoint="http://example.test/api")
    runtime.get_memory()
...
def test_get_memory_parses_memory_entries(monkeypatch):
    ...
    runtime = AgentRuntime("sess_1", endpoint="http://example.test")
...
def test_missing_endpoint_makes_no_http_calls(monkeypatch):
